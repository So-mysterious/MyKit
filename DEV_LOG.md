...
### Excel 解析器持续增强 (`lib/bookkeeping/importers/standardParser.ts`)

#### 灵活表头解析
- **列映射机制**：从位置匹配改为名称匹配
  - 新增 `ColumnMap` 接口，存储列名到索引的映射
  - 必填列：`日期`, `类型`, `金额`, `账户`
  - 可选列：`分类`, `备注`, `对方账户`, `对方金额`
  - 列顺序不限，只要表头名正确即可识别

#### 标签类型验证
- **问题**：导入"收入 交通"未报错，但"交通"是支出标签
- **修复**：
  - 创建 `tagsByKind` 分组：`{ expense: Set, income: Set, transfer: Set }`
  - 验证时检查标签是否属于正确的交易类型
  - 错误提示：`「交通」不是收入类型的标签`
...
| 输入格式 | 示例 | 解析结果 |
|---------|------|---------|
| YYYY-MM-DD | `2025-12-10` | `2025-12-10` |
| YYYY/MM/DD | `2025/12/10` | `2025-12-10` |
| **MM/DD/YY** | `10/31/25` | `2025-10-31` |
| **MM/DD/YYYY** | `10/31/2025` | `2025-10-31` |
| 带时间 | `9/27/25 15:58` | `2025-09-27T15:58:00` |

#### 符号标准化 (`normalizeSymbols`)
- **移除空白**：空格、制表符、全角空格 (`\u3000`)
- **中英文标点**：`（）【】｛｝，。：；？！` → `()[]{},.;:?!`
- **引号统一**：`""「」『』''` → `"'`
- **破折号**：`—–－` → `-`
- **全角转半角**：`０-９`, `Ａ-Ｚａ-ｚ` → `0-9`, `A-Za-z`
- **大小写**：统一转小写

**效果**：`中银 （7603）` ✅ 匹配 `中银（7603）`

#### 关键变量
- `ColumnMap`：列索引映射接口
- `normalizeSymbols()`：符号标准化函数
- `normalizedAccountMap`：标准化后的账户映射
- `tagsByKind`：按类型分组的标签集合
